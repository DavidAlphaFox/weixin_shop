...
-define(error_coin, 2).           %% 礼券不足
-define(error_item, 3).           %% 未知道具
-define(error_gold, 4).           %% 金币不足
...
-define(error_self_horn, 43).          %% 当前不能使用召唤之力
...
-define(error_redbag_num, 52).              %% 最多可以同时拥有2个红包
-define(error_help_num, 53).              %% 每天助力次数用完了
-define(error_help_max, 54).              %% 该红包只能被助力5次
-define(error_redbag_status, 55).              %% 该红包已经被领走了，不能助力了
-define(error_redbag_exsit, 56).              %% 该红包不存在了
-define(error_discount_time, 57).              %% 折扣使用权到期了
