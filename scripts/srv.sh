...
fun_sync_proto() {
    cd $ROOT/..
    cp include/error_msg.hrl ../../program/wx_shop/main/error_msg/
    cp protobuffs/proto/*.proto ../../program/wx_shop/main/proto/
    cd ../../program/wx_shop/main && svn up && svn ci -m ""
}
...
fun_start(){
    ...
    start_file=./start.sh
	CMD="erl -pa ebin +P 1024000 +K true -setcookie $COOKIE -hidden -kernel inet_dist_listen_min $inet_dist_listen_min inet_dist_listen_max $inet_dist_listen_max -name $nodename -s main start -extra $type"
    cat > ${start_file} <<EOF
#!/bin/bash
ulimit -SHn 102400
${CMD}
