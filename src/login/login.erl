...
%% 微信登陆
weixin_login({Code, ParentID, RedCode, PayCode, RedBagId, RoleId, Flag}, State) ->
             ...
             case get_weixin_RedId(RedCode) of
                 {ok, RedId} ->
                     case get_weixin_PayId(PayCode) of
                         {ok, PayId} ->
                             do_login({OpenID, NickName, Icon, Sex, ParentID, RedId, PayId, RedBagId, RoleId, Flag}, State);
                         _ ->
                             {false, ?error_busy}
                    end;
                _ ->
...
%% 手机验证码登陆
phone_screat_login({Phone, PhoneScreat, ParentID, RedBagId}, State = #conn{ip = IP}) ->
    case db:get_one("select role_id from role where phone = ?", [Phone]) of
        ...
        {ok, undefined} ->
            ...
            create({"", Name, "", 0, ParentID, "", "", RedBagId, Phone, PhoneScreat}, State);
        _Err ->
            ?ERR("数据库读取玩家数据错误~w", [_Err]),
            {false, ?error_busy}
    end.
...
do_login({OpenId, NickName, Icon, Sex, ParentID, RedId, PayId, RedBagId, 0, _Flag}, State = #conn{ip = IP}) ->
    case db:get_one("select role_id from role where openid = ?", [OpenId]) of
        ...
        {ok, undefined} ->
            create({OpenId, NickName, Icon, Sex, ParentID, RedId, PayId, RedBagId, "", ""}, State);
        _Err ->
            ?ERR("数据库读取玩家数据错误~w", [_Err]),
            {false, ?error_busy}
    end;
...
%% 创建一个新玩家
create({OpenId, NickName, Icon, Sex, ParentID, RedId, PayId, _RedBagId, Phone, PhoneScreat}, State = #conn{ip = Ip}) ->
    ...
    case role_data:new_to_db(Role) of
        ok ->
            ...
            {ok, role_conver:to_login_role(Role), Screat, State#conn{pid_object = Pid, account = RoleId}};
        _ ->
            {false, ?error_busy}
    end.
