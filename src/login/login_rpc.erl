...
%% 微信登陆
handle(1001, #m_1001_tos{code = Code, parent_id = ParentId, red_code = RedCode, pay_code = PayCode, gift_id = RedId, role_id = RoleId, flag = Flag}, State) ->
    case login:weixin_login({Code, ParentId, RedCode, PayCode, RedId, RoleId, Flag}, State) of
        {ok, Info, Id, NewState} ->
            case shop_gift_mgr:get_help_info({ParentId, RedId}) of
                {ok, Info} ->
                    {ok, #m_1001_toc{info = Info#p_role_info{gift_info = Info}, id = Id}, NewState};
                _ ->
...
%% 手机验证码登陆
handle(1006, #m_1006_tos{phone = Phone, screat = Screat, id = Code, parent_id = ParentId, gift_id = RedId}, State) ->
    case is_list(Screat) andalso Screat =/= "" of
        true ->
            case get(phone_code) of
                {Code, Phone, _} ->
                    case login:phone_screat_login({Phone, Screat, ParentId, RedId}, State) of
                        {ok, Info, Id, NewState} ->
                            case shop_gift_mgr:get_help_info({ParentId, RedId}) of
                                {ok, RedBagInfo} ->
                                    {ok, #m_1006_toc{info = Info#p_role_info{gift_info = RedBagInfo}, id = Id}, NewState};
                                _ ->
