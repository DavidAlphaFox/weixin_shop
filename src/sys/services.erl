...
%% 游戏区
cfg(local) ->
    {ok, [shop_mgr, shop_gift_mgr, order_account_mgr, account_mgr, role_account_mgr, role_charge_mgr, setting_mgr, weixin_mgr,  boradcast_mgr, log_mgr, rank_mgr, role_data,  auto_increment, web_mgr, sup_acceptor, sys_listener]};
...
get(shop_mgr = Id) ->
    {ok, #service{
            id = Id
            ,name = "商场进程"
            ,mfa = {shop_mgr, start_link, []}
        }
    };

get(shop_gift_mgr = Id) ->
    {ok, #service{
            id = Id
            ,name = "商场礼包进程"
            ,mfa = {shop_gift_mgr, start_link, []}
        }
    };
...
get(order_account_mgr = Id) ->
    {ok, #service{
            id = Id
            ,name = "订单记录"
            ,mfa = {order_account_mgr, start_link, []}
        }
    };
