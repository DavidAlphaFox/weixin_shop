...
%% 获取近期登陆信息
get_near_login() ->
    case catch gen_server:call(?MODULE, get_near_login) of
        {ok, List} ->  
            to_p_near_role(List);
        _ -> []
    end.

to_p_near_role(#online_role{role_id = RoleID, name = Name, icon = Icon}) ->
    #p_near_role{role_id = RoleID, name = Name, icon = Icon, luck = 0};
to_p_near_role(List) when is_list(List) -> 
    [to_p_near_role(R)|| R<-List].
...
%% 第一次创建角色
new_to_db(Role = #role{role_id = RoleID, name = Name, icon = Icon, open_id = OpenID, regist_time = RegistTime, gold = Gold, coin = Coin, exchange = Exchange, parent_id = ParentId, login_time = LoginTime, off_time = OffTime, charge = Charge, pay_openid = PayOpenId, red_openid = RedOpenId, phone = Phone, phone_screat = PhoneScreat}) ->
    case db:exec("insert into role (role_id, openid, icon, name, regist_time, gold, coin, parent_id, login_time, off_time, charge, exchange,  pay_openid, red_openid, phone, phone_screat, info) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [RoleID, OpenID, Icon, Name, RegistTime, Gold, Coin, ParentId,LoginTime, OffTime, Charge, Exchange,PayOpenId, RedOpenId, Phone, PhoneScreat, util:term_to_string(Role)]) of
        ok ->
            save(Role),
            ok;
        {error, Err} ->
...
%% 保存数据到数据库
save_to_db(Role = #role{role_id = RoleID, name = Name, icon = Icon,  open_id = OpenID, regist_time = RegistTime, gold = Gold, coin = Coin, exchange = Exchange, parent_id = ParentId, login_time = LoginTime, off_time = OffTime, charge = Charge, phone = Phone, phone_screat = PhoneScreat, off = Off, pay_openid = PayOpenId, red_openid = RedOpenId}) ->
    Dets = get_role_dets_name(RoleID),
    dets:insert(Dets, Role),
    log_db:log(role, insert, [RoleID, OpenID, Icon, Name, RegistTime, Gold, Coin, ParentId, LoginTime, OffTime, Charge, Phone, PhoneScreat, Off, Exchange, PayOpenId, RedOpenId, util:term_to_string(Role)]).
...
to_web_role(#role{role_id = RoleID, name = Name, coin = Coin, gold = Gold, charge = Charge, exchange = Exchange, login_time = LoginTime, off_time = OffTime, regist_time = RegistTime, phone = Phone}) ->
    {ok, [{role_id, RoleID}, {name, Name}, {coin, Coin}, {gold, Gold}, {charge, Charge}, {login_time, LoginTime}, {off_time, OffTime}, {regist_time, RegistTime}, {phone, Phone}, {exchange, Exchange}]}.
