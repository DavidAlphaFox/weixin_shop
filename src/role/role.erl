...
%% 礼券发生变化
do_coin_change(Cmd, _NewRole = #role{role_id = RoleID, coin = Value1}, _Role = #role{coin = Value2}) when Value1 =/= Value2-> 
    do_coin_log(Cmd, RoleID, Value2, Value1 - Value2, Value1);
...
%% 礼券日志处理
do_coin_log(Cmd, RoleId, Value1, Cost, Value2) ->
    Type = get_cmd_type(Cmd),
    log_db:log(coin_cost_log, insert, [RoleId, Type, Value1, Cost, Value2, date:unixtime()]).
